# 🔥👏 Consulting-App

A Consulting app, created with Flutter and Laravel 💻

## Getting Started

CONSULTATION is an electronic consulting platform through which consultations can be provided/reserved in all different fields.

Star ⭐ the repo to support the project.

## Demo

https://user-images.githubusercontent.com/103205476/224577283-b51d3022-0331-41fb-9f8e-4f4e4377aae8.mp4

## Features

- The application allows registration as a user, expert, or even as a guest to browse consultations without making any purchases.

- Browse all the different consultations provided by experts within the application.

- Add consultations to the favorites list.

- Browse personal information, whether it is a user or an expert, with the ability to log out.

- The user and the expert can book and purchase a service from another expert and transfer the amount from the customer's wallet to the expert's wallet.

- A specific service or consultation is booked within specific dates, in which the expert can work during the month.

- Flexibility in choosing the expert's appointments, as he can choose the working days in which he works, in addition to choosing more than one working time within one day.

- Through a service, the user can browse his personal page and see the expert's details.

- The expert can be evaluated and the average ratings for each expert can be displayed in each consultation, but for evaluation, at least one service must be purchased from this expert.

- The expert can see the dates of the reservations that have been purchased and see the personal pages of the booked users.

- Search for a specific rating or expert.

- Arabic and English languages are supported in the application.

- The possibility of communicating with the expert through messages.

- The possibility of making a video call between the customer and the expert so that the appointment between them takes place remotely (online).

## Screenshots

<p float="left">
  <img src="screenshots/Screenshot_20230312_004249_Gallery.jpg" width="220" height = "450"/>
  <img src="screenshots/Screenshot_20230312_004300_Gallery.jpg" width="220" height = "450"/> 
  <img src="screenshots/Screenshot_20230312_004314_Gallery.jpg" width="220" height = "450"/>
</p>

<p float="left">
  <img src="screenshots/Screenshot_20230312_004324_Gallery.jpg" width="220" height = "450"/>
  <img src="screenshots/Screenshot_20230312_004336_Gallery.jpg" width="220" height = "450"/> 
  <img src="screenshots/Screenshot_20230312_002148.jpg" width="220" height = "450"/>
</p>

<p float="left">
  <img src="screenshots/Screenshot_20230312_004410_Gallery.jpg" width="220" height = "450"/>
  <img src="screenshots/Screenshot_20230312_004447_Gallery.jpg" width="220" height = "450"/> 
  <img src="screenshots/Screenshot_20230312_004516_Gallery.jpg" width="220" height = "450"/>
</p>

<p float="left">
  <img src="screenshots/Screenshot_20230312_004532_Gallery.jpg" width="220" height = "450"/>
  <img src="screenshots/Screenshot_20230312_004635_Gallery.jpg" width="220" height = "450"/> 
  <img src="screenshots/Screenshot_20230312_004613_Gallery.jpg" width="220" height = "450"/>
</p>

<p float="left">
  <img src="screenshots/Screenshot_20230312_004651_Gallery.jpg" width="220" height = "450"/>
  <img src="screenshots/Screenshot_20230312_005110.jpg" width="220" height = "450"/> 
  <img src="screenshots/Screenshot_20230312_004717_Gallery.jpg" width="220" height = "450"/>
</p>

## Tools

- Front-end: Dart - Flutter framework
- Back-end: Php - Laravel framework
- State Management: Bloc
- Data Networking: Dio package

## Questions?🤔

Hit me on

[![LinkedIn](https://user-images.githubusercontent.com/35039342/55471530-94b34280-5627-11e9-8c0e-6fe86a8406d6.png)](https://www.linkedin.com/in/bassam-jawish/)

## Contributing

 [@batoul-khadamaljame] ([https://www.github.com/username](https://github.com/batoul-khadamaljame))


Pull requests are welcome.
